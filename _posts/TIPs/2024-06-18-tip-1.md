...
1. Context
2. System model
3. A good CRO snapshot?
4. Problem definition
5. A leader-based algorithm
6. Issues
7. Threshold Logical Clock (TLC)
8. The TLC contract
9. TLC-based consensus
10. A leaderless snapshot algorithm
11. Issues
12. Next
...
Vidigueira's notion of compatibility means that for process $i$ in round $r$, if a proposal $P_r$ is decided, then $i$'s proposal for $r+1$ or $P_{r+1}$ must build on top of $P_r$. In other words, ${P_r+1}$ causally depends on $P_r$. This is equivalent to in blockchains a new block is always proposed on top of the main (e.g. longest) chain.
